...
PI_IP=192.168.1.9
ZIP_EXPORT=fom-client.zip
HOME_DIR=/home/pi
PI_DIR=fom

echo "Adding files to ZIP export"
zip -r $ZIP_EXPORT src/client/. -x '*__pycache__*' deploy.sh
zip -g $ZIP_EXPORT ".env"
zip -g $ZIP_EXPORT src/client/requirements.txt
...
then
    ...
    # ssh pi@$PI_IP pip3 install -r $HOME_DIR/$PI_DIR/src/client/requirements.txt
else
    rm $ZIP_EXPORT
    exit 0
fi
